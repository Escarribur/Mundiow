...
public class SimularTurno {
    public static void turno(Cancha prueba1, Pelota pelota)
    {
        ...
        for(int i=0;i<22;i++)
        {
            ...
            if(Partido.Disparo(prueba1, (int)dis[i][2], (int)dis[i][1], defensor, QuienDefiende.quien(prueba1.getEquipoX(atacante).getJugadorX((int)dis[i][1]), prueba1.getEquipoX(defensor)), pelota)==true)
            {
